...
(deftest create-new-bookings
  (let [{:keys [booking-id] :as booking1} (b-db/create-booking booking-call) ;creates new booking
        ...
        _ (b-db/update-booking update-call) ;updates previously made booking
        updated-booking (db/find-booking-by-id booking-id)
        cancelled? (:cancelled updated-booking)]
    (is (s/valid? ::b-c/valid-booking booking-call))
    (is (s/valid? ::b-c/valid-booking found-booking))
    (is (s/valid? ::b-c/valid-booking updated-booking))
...
(deftest test-booking-db
  (let [vendor1 1234
    ...
    (dorun (map b-db/create-booking [booking5 booking2 booking4 booking3 booking1]))
    (is (s/valid? ::b-c/valid-booking booking1))
    (is (s/valid? ::b-c/valid-booking booking2))
    (is (s/valid? ::b-c/valid-booking booking3))
    (is (s/valid? ::b-c/valid-booking booking4))
    (is (s/valid? ::b-c/valid-booking booking5))
